...
סרטונים שמוקדשים לדיון על React ועל האקוסיסטם של React.
...
ב-keynote שיתפנו את החזון שלנו לעתיד של React, החל מ-React 18.

צפו כאן בהרצאת ה-keynote המלאה של [Andrew Clark](https://twitter.com/acdlite), [Juan Tejada](https://twitter.com/_jstejada), [Lauren Tan](https://twitter.com/potetotes) ו-[Rick Hanlon](https://twitter.com/rickhanlonii):
...
להדגמה של שדרוג ל-React 18, ראו את ההרצאה של [Shruti Kapoor](https://twitter.com/shrutikapoor08) כאן:
...
React 18 כוללת גם שיפורים לביצועי רינדור בצד השרת באמצעות Suspense.

Streaming server rendering מאפשר לייצר HTML מקומפוננטות React בשרת, ולהזרים את ה-HTML למשתמשים. ב-React 18 אפשר להשתמש ב-`Suspense` כדי לפרק את האפליקציה ליחידות עצמאיות קטנות יותר, שאפשר להזרים בנפרד בלי לחסום את שאר האפליקציה. המשמעות היא שהמשתמשים יראו את התוכן מוקדם יותר ויוכלו להתחיל אינטראקציה מהר יותר.

לצלילה עמוקה יותר, ראו את ההרצאה של [Shaundai Person](https://twitter.com/shaundai) כאן:
...
לקראת React 18 יצרנו את Working Group הראשון שלנו, כדי לשתף פעולה עם פאנל של מומחים, מפתחים, מתחזקי ספריות ואנשי חינוך. יחד בנינו את אסטרטגיית האימוץ ההדרגתי ושייפנו APIs חדשים כמו `useId`, `useSyncExternalStore` ו-`useInsertionEffect`.
...
כדי לתמוך בפיצ'רים החדשים בשחרור הזה, הכרזנו גם על צוות React DevTools החדש ועל Timeline Profiler חדש כדי לעזור למפתחים לדבג אפליקציות React.

למידע נוסף ולהדגמה של הפיצ'רים החדשים ב-DevTools, ראו את ההרצאה של [Brian Vaughn](https://twitter.com/brian_d_vaughn):
...
במבט לעתיד הרחוק יותר, [Xuan Huang (黄玄)](https://twitter.com/Huxpro) שיתף עדכון ממחקר React Labs שלנו על קומפיילר שמבצע auto-memoization. לצפייה במידע נוסף ובהדגמת אב-טיפוס של הקומפיילר:
...
[Rachel Nabors](https://twitter.com/rachelnabors) פתחה מקבץ הרצאות על למידה ועיצוב עם React באמצעות keynote על ההשקעה שלנו בתיעוד החדש של React ([כיום זמין כ-react.dev](/blog/2023/03/16/introducing-react-dev)):
...
**וגם הרצאות מהקהילה על נגישות, כלי פיתוח ו-Server Components:**
...
## סרטונים ישנים יותר {/*older-videos*/}

### React Conf 2019 {/*react-conf-2019*/}

פלייליסט סרטונים מ-React Conf 2019.
...
פלייליסט סרטונים מ-React Conf 2018.
...
פלייליסט סרטונים מ-React.js Conf 2017.
...
פלייליסט סרטונים מ-React.js Conf 2016.
...
פלייליסט סרטונים מ-React.js Conf 2015.
...
ההרצאה של Pete Hunt ב-JSConf EU 2013 עוסקת בשלושה נושאים: ויתור על רעיון התבניות ובניית views עם JavaScript, "רינדור מחדש" של כל האפליקציה כשנתונים משתנים, ומימוש קל משקל של DOM ואירועים (2013 - 0h30m).
...
Tom Occhino ו-Jordan Walke מציגים את React ב-Facebook Seattle (2013 - 1h20m).
