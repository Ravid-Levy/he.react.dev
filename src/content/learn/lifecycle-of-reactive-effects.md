...
**זו דרך טובה לחשוב על רכיבים, אבל _לא_ על אפקטים.** במקום זאת, נסה לחשוב על כל אפקט באופן עצמאי ממחזור החיים של הרכיב שלך. אפקט מתאר כיצד [לסנכרן מערכת חיצונית](/ללמוד/סנכרון-עם-אפקטים) לprops ולstate הנוכחיים. ככל שהקוד שלך משתנה, הסנכרון יצטרך להתרחש לעתים קרובות יותר או פחות.
...
זה עשוי להאמין לך איך אתה לא אם רכיב מועלה או מתעדכן שאתה כותב את לוגיקת הרינדור יוצר JSX. אתה מתאר מה צריך להיות על המסך, ומגיב [מבין את השאר.](/learn/reacting-to-input-with-state)

### How React מאמת שהאפקט שלך יכול להסתנכרן מחדש {/*how-react-verifies-that-your-react-can-re-synchronize*/}
...
### How React יודע שהוא צריך לסנכרן מחדש את האפקט {/*how-react-יודע-שצריך-לחדש-לסנכרן-את-האפקט*/}
...
לדוגמה, אם עברת את `["כללי"]`בהמשך הרנדור הראשוני, ובהמשך עברת את `["נסיעות"]`על הרינדור הבא, הגיבו ישווה את `"כללי"` ו`"נסיעות"`. אלו הם ערכים שונים (בהשוואה ל-[`Object.is`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object/is)), אז React יסנכרן מחדש את האפקט שלך. מצד שני, אם הרכיב שלך מעבד מחדש אבל `roomId` לא השתנה, האפקט שלך יישאר מחובר לאותו החדר.
...
בדוגמה שלמעלה, מחיקת אפקט אחד לא תשבור את ההיגיון של האפקט השני. זו אינדיקציה טובה בגלל שהם מסנכרנים דברים שונים, זה היה הגיוני לפצל אותם. מצד שני, אם תפצל פיסת היגיון מגובשת לאפקטים נפרדים, הקוד עשוי להיראות "נקי" יותר אבל [קשה יותר לתחזוקה.](/learn/you-might-not-need-an-effect#chains-of-computations) אתה צריך לחשוב אם התהליכים האלה או נפרדים, לא אם הקוד נראה נקי יותר.

## אפקטים "מגיבים" לערכים תגובתיים {/*effects-react-to-reactive-values*/}
...
**ערך ניתן לשינוי כמו [`ref.current`](/reference/react/useRef#reference) או שאתה קורא ממנו גם לא יכול להיות תלות.** דברים רפר שמוחזר על ידי `useRef` עצמו יכול להיות תלות, אבל המאפיין `current` הוא יכול לשינוי בכוונה. זה יכול לך להפעיל רינדור מחדש.](/learn/referencing-values-with-refs)
...
**עם זאת, אתה יכול במקום זאת "להוכיח" ל-Linter שערכים אלה הם ערכים תגוב,** כלומר שהם *לא יכולים* להשתנות כמו עיבוד מחדש. לדוגמה, אם `serverUrl` ו`roomId` אינם תלויים בעיבוד ותמיד יש להם ערכים, אתה יכול להעביר אותם מחוץ לרכיב. עכשיו הם לא צריכים להיות תלות:
...
**אתה לא יכול "לבחור" את התלות שלך.** התלות שלך חייבת לכלול את כל [ערך תגובתי](#all-variables-declared-in-the-component-body-are-reactive) אתה קורא באפקט. הליטר אוכף את זה. לפעמים זה יכול להוביל לבעיות כמו אינסופיות ולסנכרון מחדש של האפקט שלך בטווח מדי. אל תתקן את הבעיות הללו על ידי דיכוי ה-linter! הנה מה לנסות במקום:

* **בדוק שהאפקט שלך מייצג לעבוד סנכרון עצמאי.** אם האפקט שלך לא מסנכרן שום דבר, [ייתכן שהוא מיותר.](/learn/you-might-not-need-an-effect) אם הוא מסנכרן מספר דברים עצמאיים, [פיצול אותו.](#each-effect-represents-synchronization-a-separate-)
...
בשני המקרים, `canMove` הוא מחליף תגובתי שאתה קורא בתוך האפקט. יש לציין אותו ברשימת התלות של אפקט. זה מבטיח שהאפקט יסנכרן מחדש לאחר כל שינוי בערך שלו.
...
עם זאת, מסיבה כלשהי, מדרג את הstate `canMove` בתוך `handleMove` נראה "מיושן": הוא תמיד `נכון`, גם לאחר סימון תיבת הסימון. איך זה אפשרי? מצא את הטעות בקוד ותקן אותה.
...
אתה יכול לשנות את התלות של אפקט ל-`[handleMove]`, אבל אתה יכול לשנות את ההגדרה של כל רינדור. ואז האפקט *יסנכרן* מחדש אחרי כל רינדור מחדש:
...
בדוגמה זו, שירות הצ'אט ב- `chat.js` חושף שני ממשקי API שונים: `createEncryptedConnection` ו-`createUencryptedConnection`. רכיב ה-Root `App` יכול למשתמש לבחור אם להשתמש בהצפנה או לא, הוא מעביר את שיטת ה-API המקבילה לרכיב `ChatRoom` הבן בתור הפרופס של `createConnection`.
...
נכון ש-'createConnection' הוא תלות. עם זאת, קוד זה מעט שביר מה שמישהו לערוך את רכיב האפליקציה כדי להעביר פונקציה מוטבעת בתור props זה. במקרה, הערך יהיה שונה פעם אחת שרכיב האפליקציה כזה בכל מעבד מחדש, כך שהאפקט הוא להסתנכרן מחדש. כדי להשפיע, אתה מעביר את 'מוצפן' במקום זאת:
...
תראה איך פועלת תיבת הבחירה הראשונה. הוא מאכל את מצב `planetList` בתוצאה מקריאת ה-API `"/planets"`. המזהה של כוכב הלכת הנבחר כרגע נשמר בשינוי הstate `planetId`. עליך למצוא היכן להוסיף קוד נוסף כדי לשנות את הstate `placeList` יאוכלס בתוצאה של הקריאה `"/planets/" + planetId + "/places"` ל-API.
...
- תיבת הבחירה השנייה מסנכרנת לרשימת המקומות המרוחקת עבור `planetId` הנוכחי.
...
בדוק את הכרטיסייה 'useSelectOptions.js' בארגז החול כדי לראות איך זה עובד. בדרך כלל אידיאלי, רוב האפקטים באפליקציה שלך צריכים להיות מוחלפים לאחר דבר ב-Hooks מותאמים אישית, בין אם נכתבו על ידך או על ידי הקהילה. ה-Custom Hooks מסתירים את ההיגיון הסנכרון, כך שהרכיב המתקשר לא יודע על האפקט. ככלל שתמשיך לעבוד על האפליקציה שלך, תפתח פלטת הHooks לבחירה, ובסוף דבר לא תצטרך לכתוב אפקטים ברכיבים שלך שנמצאים קרוב מאוד.
